...
async def parse(video_data):
    ...
    text_extra = '|'.join([text_dict['hashtag_name'] for text_dict in video_data['text_extra']])

    video_infor = {
        'video_id': aweme_id,  # 视频ID
        'video_title': desc,  # 视频标题
        'video_keywords': text_extra,  # 视频关键词
    ...
    if video_data['images']:
        ...
        video_infor['video_url'] = video_data["video"]["play_addr"]["url_list"][0].replace("playwm", "play").replace("720",
                                                                                                                 "1080")
...
async def main():
    with open('video-id.txt', mode='r', encoding='utf-8') as f:
        id_list = f.readlines()

    video_id_list = [int(video_id) for video_id in id_list]

    # 构造视频ID列表
    # video_id_list = ['7387206019291598090']
